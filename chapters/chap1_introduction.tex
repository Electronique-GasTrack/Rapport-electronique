...
\begin{quote}
    \textit{Concevoir, réaliser et valider un système électronique intelligent et connecté permettant la mesure non-intrusive du niveau de gaz butane dans une bouteille domestique, avec affichage local et accès à distance via une application mobile.}
\end{quote}
...
\subsubsection{OS3 : Application mobile connectée}

Développer une application mobile offrant :
\begin{itemize}
    \item Connexion au backend cloud via Internet ;
    \item Consultation à distance des données transmises par la passerelle LoRa ;
    \item Historique de consommation avec graphiques ;
    \item Prédictions de durée restante basées sur les habitudes d'utilisation ;
    \item Notifications intelligentes (niveau bas, anomalies détectées).
\end{itemize}

\subsubsection{OS4 : Communication LoRa et infrastructure réseau}
...
\begin{itemize}
    ...
    \item Une passerelle LoRa optionnelle pour la remontée des données vers le cloud ;
    \item Un protocole de communication optimisé pour la faible consommation.
\end{itemize}

\subsubsection{OS5 : Sécurité et fiabilité}
...
\subsubsection{OS6 : Autonomie et efficacité énergétique}
