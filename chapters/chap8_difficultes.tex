...
\subsection{Courbe d’apprentissage de React.JS}

Le développement de l’application web avec React.JS a représenté un défi pour certains membres de l’équipe, notamment ceux n’ayant pas d’expérience préalable avec ce framework.

\textbf{Difficultés rencontrées :}
\begin{itemize}
    \item Prise en main du langage JavaScript et de la syntaxe JSX.
    \item Compréhension de la gestion de l’état (Hooks, Context) et de l’architecture des composants.
    \item Consommation des API REST et gestion asynchrone des données.
\end{itemize}
...
L’intégration entre le dispositif embarqué, l’application web et le backend a constitué l’une des phases les plus complexes du projet.

\textbf{Difficultés principales :}
\begin{itemize}
    \item Latence dans la remontée des données entre la passerelle LoRa et l'interface web.
    \item Problèmes de synchronisation des données.
    \item Différences de rendu selon les navigateurs web.
    \item Gestion des erreurs de communication avec l'API.
\end{itemize}

\textbf{Solutions adoptées :}
\begin{itemize}
    \item Multiplication des tests sur différents navigateurs.
    \item Mise en place de mécanismes de rafraîchissement automatique.
