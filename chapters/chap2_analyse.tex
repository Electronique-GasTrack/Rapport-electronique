...
\begin{enumerate}
    ...
    \item \textbf{Coût maîtrisé} : Les capteurs ultrasonores sont disponibles à des prix raisonnables (3 000 - 15 000 FCFA pièce selon la qualité).
...
Le système de détection de gaz repose sur une architecture distribuée comprenant trois dispositifs matériels indépendants communiquant via technologie LoRa longue portée :
...
\begin{enumerate}
    ...
    \item \textbf{La passerelle LoRa (optionnelle)} : Gateway permettant de transmettre les données vers un serveur cloud pour suivi à distance via application mobile ou web.
\end{enumerate}
...
Le système est principalement composé de trois sous-ensembles : \textbf{le dispositif de mesure}, \textbf{le dispositif d'affichage} et \textbf{la passerelle LoRa (optionnelle)}.
...
La passerelle LoRa optionnelle permet d'étendre les fonctionnalités du système en transmettant les données vers un serveur cloud via Wi-Fi ou Ethernet. Basée sur un ESP32, elle reçoit les trames LoRa des capteurs et les transmet à une plateforme IoT (MQTT, HTTP REST API, ou serveur personnalisé). Cela permet un suivi à distance via application mobile ou interface web, ainsi que la gestion simultanée de plusieurs bouteilles.

L'utilisateur interagit principalement avec le dispositif d'affichage pour la consultation locale, et optionnellement avec l'application mobile pour le suivi à distance, tandis que le dispositif de mesure fonctionne de manière totalement autonome avec alimentation sur batterie et gestion intelligente de la consommation énergétique.
...
\begin{table}[H]
    \centering
    \caption{Besoins fonctionnels classés par priorité}
    ...
    \begin{tabular}{p{2cm}p{4.5cm}p{9cm}}
        ...
        BF03 & Alerte niveau bas & Une alerte visuelle (LED) et sonore (buzzer) doit être déclenchée quand le niveau < 15\% \\
        ...
        \midrule
        BF06 & Passerelle IoT & Gateway LoRa permettant connexion au cloud via Wi-Fi/Ethernet \\
        BF07 & Application mobile & Interface graphique pour consultation à distance via serveur cloud \\
        BF08 & Indicateurs LED & Affichage visuel rapide de l'état sur le dispositif d'affichage (vert/orange/rouge) \\
        BF09 & Alarme sonore & Buzzer actif paramétrable pour alertes critiques \\
        BF10 & Boutons physiques & Interaction utilisateur (allumage écran, arrêt alarme) \\
        \midrule
        \multicolumn{3}{c}{\textbf{COULD HAVE (Souhaitable)}} \\
        \midrule
        BF11 & Historique cloud & Stockage de l'historique de consommation sur serveur distant \\
        BF12 & Multi-bouteilles & Gestion de plusieurs capteurs par une seule passerelle \\
        BF13 & Statistiques & Analyse de consommation avec graphiques via application \\
        BF14 & Prédiction & Estimation du nombre de jours restants basée sur historique \\
        BF15 & Notifications push & Alertes mobiles en cas de niveau bas \\
        \midrule
        \multicolumn{3}{c}{\textbf{WON'T HAVE (Non prioritaire)}} \\
        \midrule
        BF16 & Détection de fuites & Alerte en cas de fuite détectée (capteur MQ-6 additionnel) \\
        BF17 & Géolocalisation & Localisation des points de vente de bouteilles \\
        BF18 & Partage social & Partage des statistiques sur réseaux sociaux \\
        BF19 & Commande vocale & Intégration assistants vocaux (Alexa, Google Home) \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{itemize}
    ...
    \item \textbf{Accessibilité mobile} : Compatible avec Android 8.0+ et iOS 13+ (si passerelle cloud)
\end{itemize}
...
\begin{enumerate}
    ...
    \item Il installe le dispositif d'affichage à l'endroit souhaité (cuisine, salon) et insère les batteries 18650.
    ...
    \item L'utilisateur lance la procédure de calibration via le bouton dédié sur le dispositif d'affichage.
...
\begin{enumerate}
    ...
    \item Le niveau de gaz actuel s'affiche immédiatement en pourcentage (ex: 67\%) et sous forme de bargraphe.
    \item Les LEDs indiquent l'état : verte (>50\%), orange (20-50\%), rouge (<20\%).
    \item L'heure de la dernière mesure est affichée sur la deuxième ligne du LCD.
    \item Si l'utilisateur possède la passerelle cloud, il peut également consulter le niveau via l'application mobile depuis n'importe où.
...
\begin{enumerate}
    \item Le dispositif de mesure détecte lors d'une mesure périodique que le niveau est descendu sous le seuil de 15\%.
    \item Il transmet immédiatement cette information critique via LoRa au dispositif d'affichage.
    \item Le buzzer du dispositif d'affichage émet un signal sonore intermittent (3 bips toutes les 10 secondes).
    \item La LED rouge clignote de manière continue.
    \item L'écran LCD affiche "NIVEAU BAS - 12\%" avec une icône d'avertissement.
    \item Si la passerelle cloud est installée, une notification push est envoyée sur le smartphone de l'utilisateur.
    \item L'application mobile affiche un message suggérant de prévoir un rechange rapidement.
    ...
    \item L'alarme se réactivera automatiquement après 6 heures si le niveau n'a pas augmenté.
\end{enumerate}

\subsubsection{Scénario 4 : Suivi à distance via passerelle cloud (optionnel)}

\begin{enumerate}
    \item L'utilisateur a installé la passerelle LoRa ESP32 connectée à son réseau Wi-Fi domestique.
    \item La passerelle reçoit périodiquement les trames LoRa du capteur (toutes les 15 minutes).
    \item Elle transmet ces données au serveur cloud via protocole MQTT ou HTTP REST API.
    \item L'utilisateur ouvre l'application mobile même en étant absent de son domicile.
    \item Il accède à l'onglet "Tableau de bord" qui affiche le niveau actuel de toutes ses bouteilles.
    \item Il consulte l'onglet "Historique" pour voir l'évolution de la consommation sur les 7 derniers jours.
    \item L'application affiche des graphiques de consommation quotidienne et hebdomadaire.
    \item Il accède ensuite à l'onglet "Prédiction".
    \item Un algorithme basé sur la consommation moyenne estime le nombre de jours restants avant épuisement complet.
    \item L'application suggère une date optimale pour le remplacement de la bouteille.
    \item L'utilisateur peut configurer les seuils d'alerte et la fréquence de mesure via l'application.
\end{enumerate}

\subsubsection{Scénario 5 : Gestion de plusieurs bouteilles (avec passerelle)}

\begin{enumerate}
    \item Un utilisateur possède deux bouteilles : une en utilisation, une de rechange.
    \item Il installe un capteur sous chaque bouteille (chaque capteur a un identifiant unique).
    \item La passerelle LoRa reçoit les données des deux capteurs sur le même canal 433 MHz.
    \item L'application mobile affiche deux widgets distincts : "Bouteille Cuisine" et "Bouteille Rechange".
    \item Quand la bouteille principale atteint 10\%, l'application suggère : "Préparez le remplacement - Rechange disponible à 92\%".
    \item L'utilisateur peut renommer, archiver, ou réassigner les capteurs via l'interface.
    \item Le système conserve l'historique séparé de chaque bouteille pour analyse de consommation.
\end{enumerate}
...
\begin{itemize}
    \item \textbf{Budget prototype global} : Maximum 200 000 FCFA pour système complet sans passerelle
    \item \textbf{Budget avec passerelle} : Maximum 300 000 FCFA incluant gateway ESP32
    \item \textbf{Répartition budgétaire} :
    \begin{itemize}
        \item Dispositif de mesure : 20 000 - 40 000 FCFA
        \item Dispositif d'affichage : 20 000 - 40 000 FCFA
        \item Passerelle LoRa (opt) : 15 000 - 30 000 FCFA
        \item PCB et boîtiers : 10 000 - 20 000 FCFA
    \end{itemize}
    ...
    \item \textbf{Coût cible production} : Coût unitaire de production série inférieur à 60 000 FCFA
    \item \textbf{Maintenance} : Coût de maintenance annuel inférieur à 2 000 FCFA (remplacement batteries)
\end{itemize}
...
\begin{center}
    \textbf{TOTAL :} \textbf{26 semaines} (avec passerelle cloud)\\
    \textbf{TOTAL :} \textbf{24 semaines} (version standalone sans cloud)
\end{center}
