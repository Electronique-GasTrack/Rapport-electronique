...
\begin{enumerate}
    ...
    \item \textbf{La passerelle LoRa (optionnelle)} : Gateway permettant de transmettre les données vers un serveur cloud pour suivi à distance via application web.
\end{enumerate}
...
L'utilisateur interagit principalement avec le dispositif d'affichage pour la consultation locale, et optionnellement avec l'application web pour le suivi à distance, tandis que le dispositif de mesure fonctionne de manière totalement autonome avec alimentation sur batterie et gestion intelligente de la consommation énergétique.
...
\begin{table}[H]
    ...
    \begin{tabular}{p{2cm}p{4.5cm}p{9cm}}
        ...
        BF07 & Application web & Interface graphique pour consultation à distance via serveur cloud \\
        ...
        BF15 & Notifications & Alertes par email ou interface web en cas de niveau bas \\
        \midrule
...
\begin{itemize}
    ...
    \item \textbf{Accessibilité web} : Compatible avec tout navigateur web moderne (Chrome, Firefox, Safari)
\end{itemize}
...
\begin{enumerate}
    ...
    \item Si l'utilisateur possède la passerelle cloud, il peut également consulter le niveau via l'application web depuis n'importe où.
...
\begin{enumerate}
    ...
    \item Si la passerelle cloud est installée, une alerte est visible sur le tableau de bord web.
    \item L'application web affiche un message suggérant de prévoir un rechange rapidement.
...
\begin{enumerate}
    ...
    \item L'utilisateur ouvre l'application web via son navigateur même en étant absent de son domicile.
...
\begin{enumerate}
    ...
    \item L'application web affiche deux widgets distincts : "Bouteille Cuisine" et "Bouteille Rechange".
