...
\begin{itemize}
    ...
    \item \textbf{La passerelle LoRa (Optionnelle)} : Cet élément permet d'ouvrir le système vers l'Internet des Objets (IoT). Elle fait le pont entre le réseau local LoRa et le Cloud, permettant ainsi l'usage de l'application web.
\end{itemize}
...
L'application web constitue le terminal de supervision avancé. Elle ne communique pas directement avec les capteurs, mais interroge le serveur Cloud (Backend).

Son rôle dépasse le simple affichage : elle transforme les données brutes en informations utiles. Elle permet notamment de visualiser l'historique de consommation, de consulter les alertes en cas de niveau bas où que l'on soit, et d'utiliser des algorithmes prédictifs pour estimer la date de remplacement de la bouteille.

\subsection{Architecture du frontend}

L'architecture de l'application repose sur le framework \textbf{React.js}, suivant le modèle SPA (Single Page Application). Cette approche permet une expérience utilisateur fluide, sans rechargement complet de la page lors de la navigation.

L'application est structurée autour de composants réutilisables :
\begin{enumerate}
    \item \textbf{Les Composants d'Interface (UI)} : Boutons, graphiques, cartes d'information qui constituent les briques visuelles de l'application.
    \item \textbf{La Gestion d'État (State Management)} : Utilisation des Hooks (useState, useEffect) et du Context API pour gérer les données globales (utilisateur connecté, liste des bouteilles) de manière efficace.
    \item \textbf{Le Routage} : Gestion de la navigation entre les différentes vues (Tableau de bord, Historique, Paramètres) via React Router.
    \item \textbf{La Couche Service} : Modules dédiés à la communication avec l'API REST du backend (via Axios), assurant la récupération et l'envoi sécurisé des données.
\end{enumerate}
...
\begin{enumerate}
    \item \textbf{Les Contrôleurs REST} gèrent les entrées/sorties. Ils reçoivent les mesures de la passerelle et répondent aux requêtes de l'application web, tout en sécurisant les accès via des jetons JWT.
...
\begin{itemize}
    ...
    \item \textbf{Notifications} : Gestion des alertes pour l'interface web
    \item \textbf{API REST documentée} : Endpoints exposés avec Swagger/OpenAPI pour l'app web
...
Pour l'interface utilisateur web, nous avons choisi la bibliothèque React.js.

\textbf{Justification du choix :}
\begin{itemize}
    \item \textbf{Architecture à base de composants} : Facilite la réutilisation du code et la maintenance.
    \item \textbf{DOM Virtuel} : Optimise les performances d'affichage, crucial pour les graphiques en temps réel.
    \item \textbf{Écosystème riche} : Disponibilité de nombreuses bibliothèques pour les graphiques (Recharts, Chart.js) et le design (Material UI, Tailwind).
    \item \textbf{Popularité et support} : Une large communauté et une documentation exhaustive facilitant le développement.
\end{itemize}

\vspace{1pt}

\textbf{Fonctions principales du frontend :}
\begin{itemize}
    \item \textbf{Tableau de bord de supervision} : Affichage synthétique des niveaux de gaz et statuts des capteurs (batterie, connectivité).
    \item \textbf{Visualisation analytique} : Restitution graphique de l'historique de consommation via des courbes interactives.
    \item \textbf{Gestion de compte} : Interface de connexion, inscription et gestion de profil utilisateur.
    \item \textbf{Configuration des alertes} : Définition personnalisée des seuils de notification pour chaque bouteille.
    \item \textbf{Communication API} : Abstraction des appels REST vers le backend avec gestion des erreurs et des chargements.
    \item \textbf{Adaptabilité} : Interface responsive garantissant une expérience utilisateur optimale sur mobile et desktop.
\end{itemize}
\vspace{1pt}

\textbf{Stack technique utilisée :}
\begin{itemize}
    \item \textbf{Langage} : JavaScript (ES6+) / JSX
    \item \textbf{Framework UI} : React 18
    \item \textbf{Routage} : React Router v6
    \item \textbf{Client HTTP} : Axios
    \item \textbf{Gestion d'état} : Context API + Hooks
